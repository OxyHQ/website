...
- **Pinned ecosystem versions (2026-06-11)**: `expo ~56.0.11` + `@oxyhq/services 6.10.7` + `@oxyhq/bloom ^0.6.11/0.6.12`.
...
- `@oxyhq/core` **3.4.1** — 3.4.1: identity-scoped HttpService GET response cache — cache keys now include a per-identity tag (decoded userId/acting-as, `anon` fallback); dedupe key shares the same scoping; `clearTokens()` clears the cache on logout. Fixes bug where an anonymous cold-boot GET (e.g. `/profiles/recommendations`) was cached and served to the later authenticated refetch → personalized filtering (exclude self + already-followed) never applied → "Who to follow" recommended already-followed accounts. `@oxyhq/contracts@0.1.0` also published to npm. **GOTCHA: 3.3.0 and 3.4.0 are BROKEN for external consumers** — they declared `"@oxyhq/contracts": "workspace:*"` (unresolvable outside the monorepo) and `@oxyhq/contracts` was unpublished, so `bun install` fails in consumer apps. Fixed in 3.4.1. Rule: never publish a package with a `workspace:*` dep on an unpublished package. Pin core to **≥3.4.1**. (3.2.0: additive — `PublicApplication` type + `getPublicApplication(clientId)`; `OxyConfig.clientId`; **`AuthManager.getAccessToken()` falls back to in-memory `_lastKnownAccessToken`** when storage empty — cookie-restore/cold-boot left token only in memory → standalone API clients 401'd; `workspaces` mixin (CRUD+members+transfer) + `Workspace`/`WorkspaceMember` types; `getApplications(workspaceId?)` scoping; `Application.workspaceId`; `ApplicationMember._id: string|null` + `source?: 'workspace'`; `CreateApplicationInput.workspaceId`; invite inputs `{ usernameOrEmail, role }`. 3.1.0: additive — `rotatedFromCredentialId` on `ApplicationCredential`; `RotateApplicationCredentialResult` type; `ServiceApp.credentialId`; zero-downtime credential rotation with 7-day grace. 3.0.0 BREAKING: removed `developer` mixin + all `DeveloperApp` types; added `applications` mixin + `Application`/`ApplicationMember`/`ApplicationCredential`/`ApplicationRole` types. `configureServiceAuth`/`getServiceToken`/`makeServiceRequest` unchanged. 2.4.1: hard settle guarantee for FedCM silent step. 2.3.0: single source of truth for cross-domain SSO helpers)
- `@oxyhq/auth` **4.1.1** — `WebOxyProvider` intercepts `/__oxy/sso-callback` before child apps render and peers on `@oxyhq/core ^3.4.5` (4.1.0: optional `clientId` prop; 4.0.0: major bump; 3.4.0: `WebOxyProvider` consumes core SSO helpers; 3.3.0: `/sso/establish` handoff + per-apex cookie; 3.2.0: consumes `runColdBoot` + `autoDetect`; 3.0.0: major bump)
- `@oxyhq/services` **working tree has unpublished Trust screen changes** — Karma*→Trust* screen rename; BREAKING `RouteName`: removed `KarmaCenter|KarmaLeaderboard|KarmaRewards|KarmaRules|AboutKarma|KarmaFAQ`; added `TrustCenter|TrustLeaderboard|TrustRewards|TrustRules|AboutTrust|TrustFAQ`. Consumers calling `showBottomSheet('Karma...')` MUST migrate. (10.2.0: additive — details TBD. 10.1.0: native device-flow sign-in fix. 10.0.0 BREAKING — `appName` prop REMOVED from `OxyProvider`; cross-app device sign-in now REQUIRES `clientId` prop; native-safe SSO guard via `silentGuardKey.ts`; peer `@oxyhq/core ^3.2.0`. 9.0.1: `silentGuardKey.ts` RN safety — RN `window` has no `window.location`. 8.7.0: additive `appName` option on `OxyProvider` → flows to `OxyContext` and both sign-in screens (`SignInModal`, `OxyAuthScreen`) which send it as `appId` on `POST /auth/session/create`; resolution order via `resolveAppDisplayName(appName, storageKeyPrefix)`: explicit `appName` → capitalized `storageKeyPrefix` (ignored when default `oxy_session`) → `document.title` (web) → `Platform.OS`; fixes OAuth consent page showing "web"/"Sign in to web" instead of real app name (Mention #143); no `@oxyhq/core` change; Mention consumes via `^8.7.0` + explicit `appName="Mention"` in `AppProviders.tsx`. 8.6.1: wires `COLD_BOOT_OVERALL_DEADLINE = 20000`. 8.5.0: `isAuthResolved` boolean on `useAuth()`/`useOxy()`. 8.4.0: `OxyContext` consumes core SSO helpers. 8.0.0 BREAKING: `@tanstack/*` → peerDeps; `RouteName` `'AccountSettings'`/`'AccountCenter'` → `'ManageAccount'`)
- `@oxyhq/bloom` **0.7.7** — 0.7.7: fixed `applyColorPresetVars` web path writing raw HSL triples without `hsl()` wrapper → runtime transparent/unstyled on Vite+Tailwind v4+shadcn web apps (auth, console); fix: `toWebColorValue()` wraps triples in `hsl()` on all web write paths; native paths unchanged. See "Bloom Web CSS-var Format Contract" below. 0.7.6: NW5 platform-split fix. 0.7.5–0.7.4: light-mode preset tints. 0.7.2: native NW5 color vars. 0.7.3: globalThis-guard BloomThemeContext + portal group. 0.7.0: 10 new components: `popover`, `command`, `combobox`, `alert-dialog` (overlays, web-forked), `slider`, `field`, `input-group`, `label`, `kbd`, `item` (universal single-file). **Bloom web components are RN-via-react-native-web, NOT DOM/shadcn** — a DOM app (console) needs the react-native-web Vite alias to consume them (which the website Vite config already uses).

**All consumers on latest (2026-06-16):** Mention on `@oxyhq/core 3.4.1` (commit 09551664); OxyHQServices apps (accounts/console/inbox/auth), Alia, TNP, Allo, Syra, website bumped to `@oxyhq/bloom 0.7.7` + `@oxyhq/services 10.2.0` (where used) + `@oxyhq/core 3.2.0` (pre-3.4.1 bump — update as each app migrates). Homiio: bloom 0.7.7 frontend bump pushed; backend has large in-progress rewrite in working tree (services/core bump pending).

**CRITICAL — SSO helpers live ONLY in `@oxyhq/core` (2026-06-14):** `consumeSsoReturn`, `ssoBounce` helpers (`buildSsoBounceUrl`, `isCentralIdPOrigin`, `guardActive`, `ssoNavigate`, `ssoStateKey`/`ssoGuardKey`/`ssoDestKey`/`ssoNoSessionKey`, `SSO_CALLBACK_PATH`, `SSO_GUARD_TTL_MS`), `registrableApex`, `MULTIPART_TLDS`, and `CENTRAL_IDP_APEX` are defined ONCE in `@oxyhq/core` and imported by auth-sdk, services, AND the CF Worker (`packages/auth/server/index.ts`). Do NOT re-introduce local copies in any consumer — they previously drifted (`guardActive` signature and `isCentralIdPOrigin` normalization differed between copies).
