...
`scripts/prerender.ts` writes `dist/<route>/index.html` for every route it enumerates, plus `sitemap.xml`, `_redirects`, `404.html` and `app-shell.html`. The `<head>` is the real `<SEO>` component's output through the SSR bundle; markdown routes render prose through `ArticleMarkdown`, capped per document. **Procedure and the Search Console findings behind these rules: `docs/SEARCH-INDEXING.md`.**

- **A document's canonical URL ends in `/`, and so must every link to it.** Cloudflare 308s the bare form, so a react-router `<Link to="/pricing">` publishes a redirect as this site's idea of where the page is (725 URLs under "Page with redirect"). Import `Link`/`NavLink` from `src/lib/navigation`; `withDocumentTrailingSlash` is the one authority for links, canonical, hreflang, sitemap and feeds.
- **`_redirects` carries no `200` rewrite, ever.** Cloudflare matches one BEFORE the static asset: `/newsroom/*  /app-shell.html  200` shadowed every prerendered Newsroom document, `/developers/docs/*` shadowed 2,087, and the old `/*  /index.html  200` served the HOME PAGE for every retired URL. Unknown paths get `404.html`; the SPA fallback is `functions/_middleware.ts` upgrading that 404, from `src/lib/spaFallback.ts`. Neither fallback document may carry a static `robots` meta: Helmet manages only the tags it emits.
- **A locale mirror exists only where a translation does** — `src/lib/localizedRoute.ts`, one authority for `<SEO>`, sitemap and prerender. hreflang is reciprocal: it must carry the DEFAULT locale's self-reference, which `translationReady` never yields. And an unreachable `/api/locales` is not "no locales" — `_redirects` takes `localeReadinessKnown` apart, or one timeout 301s every live `/es/…` away.
- **A route serves prose only where prose is what it HAS** — newsroom posts and synced docs. A marketing page is components; a heading repeating its `<title>` is boilerplate.
- **A locale mirror keeps the shell and drops the prose**: the markdown behind it is the default locale's text, and a `/es/` URL serving English prose reads worse than none.
- The app mounts with `createRoot`, which empties `#root` first, so prerendered prose is never markup React reconciles. **Do not switch to `hydrateRoot`** without making the markup match the page tree.
- **Verify with JavaScript blocked, and mind the trailing slash** — `vite preview` answers the slashless form with the SPA fallback, so a slashless probe reads an empty shell.
