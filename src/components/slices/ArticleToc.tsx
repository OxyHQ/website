...
function Row({ entry, spacing, onNavigate }: { entry: TocEntry; spacing: string; onNavigate?: () => void }) {
  const { number, text } = splitNumber(entry.label)
  return (
    <a
      href={`#${entry.id}`}
      onClick={onNavigate}
      className={`flex-1 cursor-pointer ${spacing} text-alt-gray-e2 transition duration-100 hover:text-gray-a1 ${
...
export default function ArticleToc({ entries, pdfHref }: ArticleTocProps) {
  ...
  const pdfLink = (className: string) =>
    pdfHref && (
      <a
        ...
        className={`flex shrink-0 items-center rounded-full border border-gray-a6 px-3.5 py-2 text-b3 text-gray-a1 outline-none transition-all duration-100 ease-impulse hover:border-alt-gray-e2 ${className}`}
      >
        Download PDF
  ...
  return (
    <nav className="sticky top-[var(--header-height)] col-span-full text-b4 max-lg:z-10 max-lg:w-full max-lg:bg-gray-a10 lg:col-span-2 lg:row-span-99 lg:row-start-1 lg:top-[calc(var(--header-height)+5rem)] lg:w-[13rem] lg:min-h-0 lg:overflow-clip">
      ...
      <div className="lg:hidden">
        <div className="flex items-center gap-3 pb-2">
          <button
            ...
            className="flex flex-1 cursor-pointer items-center justify-between rounded-full border border-gray-a6 px-3.5 py-2 text-b3 text-gray-a1 outline-none"
          >
              ...
              {entries.map((entry) => {
                ...
                return (
                  <a
                    ...
                    className={`cursor-pointer py-1.5 text-start text-alt-gray-e2 transition duration-100 hover:text-gray-a1 ${
      ...
      <div className="hidden lg:block lg:max-h-[calc(100vh-var(--header-height)-5rem)] lg:overflow-x-hidden lg:overflow-y-auto [scrollbar-width:none]">
        {pdfLink('mb-8 w-fit justify-self-start bg-gray-a10')}
        {sections.map(({ entry, children }, index) => (
          <div key={entry.id}>
            <div className="flex items-center">
              ...
              <Row entry={entry} spacing={index === 0 ? 'pb-2' : 'py-2'} />
              {children.length > 0 && (
                <button
                  type="button"
                  ...
                  className="-me-3 cursor-pointer p-3 text-gray-a1/60 transition-colors duration-100 hover:text-gray-a1"
                >
                  <SliceIcon
                    name="chevron-down"
