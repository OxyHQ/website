    ...
    <div className="flex w-max">
      ...
      >
        {dropdown.sections.flatMap((section, si) => [
          <li key={`heading-${si}`} className="contents">
            <p className={`mt-3 mb-1 inline-block px-4 text-[10px] font-semibold uppercase tracking-wider text-[var(--color-caption-fg)] ${isPlatform ? 'col-span-2' : ''}`}>
              {section.heading}
            </p>
          </li>,
      ...
      {dropdown.sidePanel && (
        <ul className="relative flex w-48 shrink-0 flex-col gap-1.5 p-4 pt-3">
          <svg width="1" height="100%" className="absolute inset-y-0 left-0 text-[var(--color-primary-fg)]/20">
            <line x1="0.5" y1="0" x2="0.5" y2="100%" stroke="currentColor" strokeLinecap="round" />
          </svg>
          <li className="contents">
            <p className="mt-3 mb-0.5 inline-block px-2.5 text-[10px] font-semibold uppercase tracking-wider text-[var(--color-caption-fg)]">
          ...
          {dropdown.sidePanel.links.map((link, i) => (
            <li key={i} className="contents">
              <a
                href={link.href}
                className="inline-flex h-8 w-full items-center justify-start whitespace-nowrap rounded-[10px] border border-transparent px-2.5 text-sm text-[var(--color-primary-fg)] transition-colors duration-300 hover:bg-[var(--color-black-400)]"
              >
                {link.label}
              </a>
            </li>
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--color-subtle-stroke)] bg-[var(--color-primary-bg)] transition-colors duration-300">
      ...
      {/* ─── Banner ─── */}
      {bannerVisible && (
        <div className="isolate flex w-full items-center justify-center bg-[var(--color-black-200)]">
          <Container>
            <div className="relative flex h-10 items-center justify-center">
              <a href="#" className="group flex items-center gap-1.5 text-[13px] leading-5 text-[var(--color-primary-fg)]">
                <span>Ask more from CRM. Ask Oxy.</span>
                <ArrowRight />
              </a>
              <button
                className="absolute right-0 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-[10px] text-[var(--color-tertiary-fg)] transition-colors hover:text-[var(--color-primary-fg)]"
          ...
          <div className="flex items-center justify-between">
            <div className="flex grow items-center gap-x-9">
              <a href="/" className="-mx-1.5 rounded-xl px-1.5" aria-label="Oxy homepage">
                <span className="mb-1 block text-xl font-bold tracking-tight text-[var(--color-primary-fg)]">Oxy</span>
              </a>
              ...
              <div ref={navAreaRef} className="relative z-10" onMouseLeave={scheduleClose}>
                <ul className="hidden items-center gap-x-1.5 lg:flex">
                  {dropdowns.map((dd) => (
                    <li key={dd.label}>
                      <button
                        ...
                        className="group inline-flex h-9 cursor-pointer select-none items-center justify-center gap-x-1.5 rounded-[10px] border border-transparent px-3 text-[15px] transition-colors duration-300 hover:bg-[var(--color-black-400)] hover:text-[var(--color-white-100)]"
                        style={{
                  ...
                  {simpleNavLinks.map((link) => (
                    <li key={link.label}>
                      <a
                        href={link.href}
                        className="inline-flex h-9 items-center justify-center rounded-[10px] border border-transparent px-3 text-[15px] text-[var(--color-white-900)] transition-colors duration-300 hover:bg-[var(--color-black-400)] hover:text-[var(--color-white-100)]"
                ...
                {hasMeasured && (
                  ...
                  >
                    <div
                      className="overflow-hidden rounded-xl border border-[var(--color-subtle-stroke)] bg-[var(--color-primary-bg)] shadow-lg"
                      style={{
            ...
            {/* Mobile hamburger */}
            <button
              className="inline-flex h-9 w-9 items-center justify-center rounded-[10px] text-[var(--color-white-500)] lg:hidden"
      ...
      {/* ─── Mobile drawer ─── */}
      {mobileOpen && (
        <div className="border-t border-[var(--color-subtle-stroke)] bg-[var(--color-primary-bg)] lg:hidden">
          <Container>
            <div className="flex flex-col gap-1 py-4">
              {dropdowns.map((dd) => (
                <div key={dd.label}>
                  <button
                    className="flex w-full items-center justify-between rounded-xl px-4 py-3 text-base text-[var(--color-secondary-fg)] transition-colors hover:bg-[var(--color-surface-subtle)]"
                  ...
                  {mobileAccordion === dd.label && (
                    <div className="flex flex-col gap-1 pb-2 pl-4">
                      {dd.sections.map((s) =>
                        s.items.map((item) => (
                          <a key={item.title} href={item.href} className="rounded-xl px-4 py-2 text-sm text-[var(--color-tertiary-fg)] transition-colors hover:bg-[var(--color-surface-subtle)] hover:text-[var(--color-secondary-fg)]" onClick={() => setMobileOpen(false)}>
                            {item.title}
                          </a>
                        ))
                      )}
                      {dd.sidePanel?.links.map((link) => (
                        <a key={link.label} href={link.href} className="rounded-xl px-4 py-2 text-sm text-[var(--color-tertiary-fg)] transition-colors hover:bg-[var(--color-surface-subtle)] hover:text-[var(--color-secondary-fg)]" onClick={() => setMobileOpen(false)}>
                          {link.label}
                        </a>
                      ))}
                    </div>
              ...
              {simpleNavLinks.map((link) => (
                <a key={link.label} href={link.href} className="rounded-xl px-4 py-3 text-base text-[var(--color-secondary-fg)] transition-colors hover:bg-[var(--color-surface-subtle)]" onClick={() => setMobileOpen(false)}>
                  {link.label}
                </a>
              ))}
              <hr className="my-2 border-[var(--color-subtle-stroke)]" />
