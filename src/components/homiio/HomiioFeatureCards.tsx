...
const FEATURES: readonly Feature[] = [
  {
    ...
    icon: <Smiley weight="bold" className="h-1/3 w-1/3 text-primary-foreground" />,
    from: 'var(--primary)',
    to: 'var(--primary-subtle)',
  },
  {
    ...
    icon: <MagnifyingGlass weight="bold" className="h-1/3 w-1/3 text-secondary-foreground" />,
    from: 'var(--secondary)',
    to: 'var(--secondary-subtle)',
  },
  {
    ...
    icon: (
      <div className="relative grid h-1/3 w-1/3 place-items-center">
        <GlobeHemisphereWest weight="bold" className="h-full w-full text-tertiary-foreground" />
        <Lock weight="fill" className="absolute -bottom-1 -right-1 h-1/2 w-1/2 text-tertiary-foreground drop-shadow" />
      </div>
    ),
    from: 'var(--tertiary)',
    to: 'var(--tertiary-subtle)',
  },
]
...
function tileBackground(from: string, to: string): string {
  return [
    'repeating-linear-gradient(180deg, color-mix(in srgb, var(--foreground) 18%, transparent) 0px, color-mix(in srgb, var(--foreground) 18%, transparent) 7px, color-mix(in srgb, var(--background) 6%, transparent) 7px, color-mix(in srgb, var(--background) 6%, transparent) 14px)',
