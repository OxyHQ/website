    ...
    <section className="mx-auto w-full max-w-2xl px-6 pb-16 pt-2">
      <ul className="flex flex-col gap-2">
        {HOMIIO_FAQS.map((faq, i) => {
          const isOpen = open === i
          return (
            <li key={faq.question} className="overflow-hidden rounded-2xl bg-white shadow-[0_10px_30px_-18px_rgba(0,0,0,0.3)] ring-1 ring-black/5">
              <button
                type="button"
              ...
              <AnimatePresence initial={false}>
                {isOpen && (
                  ...
                  >
                    <p className="px-5 pb-4 text-sm leading-relaxed text-neutral-500">{faq.answer}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </li>
