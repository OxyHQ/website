import { motion } from 'framer-motion'
import { RiArrowRightUpLine } from '@oxy.so/bloom/icons/RiArrowRightUpLine'
import { RiChat3Line } from '@oxy.so/bloom/icons/RiChat3Line'
import { RiSendPlaneLine } from '@oxy.so/bloom/icons/RiSendPlaneLine'
import { RiTwitterXFill } from '@oxy.so/bloom/icons/RiTwitterXFill'

const REDDIT_URL = 'https://reddit.com/r/FairCoinOfficial'
const TELEGRAM_URL = 'https://t.me/FairCoin_'
const TWITTER_URL = 'https://twitter.com/FairCoin_'
const GITHUB_ORG_URL = 'https://github.com/FairCoinOfficial'

const ITEMS = [
  {
    name: 'Reddit',
    description: 'Discussions, AMAs and longer-form posts on r/FairCoinOfficial.',
    href: REDDIT_URL,
    icon: RiChat3Line,
  },
  {
    name: 'Telegram',
    description: 'Day-to-day chat with maintainers and the global community.',
    href: TELEGRAM_URL,
    icon: RiSendPlaneLine,
  },
  {
    name: 'Twitter',
    description: 'Announcements and short updates from the FairCoin team.',
    href: TWITTER_URL,
    icon: RiTwitterXFill,
  },
  {
    name: 'GitHub',
    description:
      'Every repository (chain, wallets, explorer, bridge) open on FairCoinOfficial.',
    href: GITHUB_ORG_URL,
    icon: GithubIcon,
  },
] as const

export default function CommunitySection() {
  return (
    <section className="relative isolate">
      <div className="container mx-auto px-4 py-20 sm:py-24">
        <div className="mx-auto max-w-2xl text-center">
          <motion.span
            initial={{ opacity: 0, y: 8 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.4 }}
            transition={{ duration: 0.4, ease: 'easeOut' }}
            className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground"
          >
            Community
          </motion.span>
          <motion.h2
            initial={{ opacity: 0, y: 12 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.4 }}
            transition={{ duration: 0.5, ease: 'easeOut' }}
            className="mt-5 text-balance text-[32px] font-semibold leading-tight tracking-tight text-foreground sm:text-[40px]"
          >
            Talk to the project
          </motion.h2>
        </div>

        <div className="mx-auto mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {ITEMS.map((item, idx) => (
            <motion.a
              key={item.name}
              href={item.href}
              target="_blank"
              rel="noopener noreferrer"
              initial={{ opacity: 0, y: 16 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{ duration: 0.5, ease: 'easeOut', delay: idx * 0.05 }}
              className="group relative flex items-center gap-4 overflow-hidden rounded-3xl border border-border bg-popover/60 p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40"
            >
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                <item.icon width={20} height={20} fill="currentColor" />
              </span>
              <div className="flex flex-1 flex-col gap-0.5">
                <h3 className="text-base font-semibold text-foreground">{item.name}</h3>
                <p className="text-sm text-muted-foreground">{item.description}</p>
              </div>
              <span aria-hidden="true" className="inline-flex text-muted-foreground transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-foreground">
                <RiArrowRightUpLine width={16} height={16} fill="currentColor" />
              </span>
            </motion.a>
          ))}
        </div>
      </div>
    </section>
  )
}

// Bloom ships no GitHub glyph, so this one keeps its own path and takes the
// same width / fill props as the Remix icons beside it in ITEMS.
function GithubIcon({ width = 20, fill = 'currentColor' }: { width?: number; height?: number; fill?: string }) {
  return (
    <svg aria-hidden width={width} height={width} viewBox="0 0 24 24" fill={fill}>
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  )
}
