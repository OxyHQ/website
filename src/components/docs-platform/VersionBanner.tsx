...
export default function VersionBanner({ pkg, currentVersion, slug = '' }: VersionBannerProps) {
  ...
  if (deprecated) {
    return (
      <div
        role="alert"
        className="not-prose mb-6 flex flex-col gap-3 rounded-2xl border border-error/40 bg-error-subtle px-4 py-3 text-sm sm:flex-row sm:items-center"
      >
        ...
        <Link
          to={latestHref}
          className="inline-flex shrink-0 items-center justify-center rounded-xl bg-error px-3 py-1.5 text-xs font-medium text-error-foreground hover:bg-error/90"
        >
  ...
  return (
    <div
      role="status"
      className="not-prose mb-6 flex flex-col sm:flex-row sm:items-center gap-3 rounded-2xl border border-warning/40 bg-warning-subtle px-4 py-3 text-sm"
    >
      <div className="flex items-start gap-2 flex-1">
        <svg
          aria-hidden
      ...
      <Link
        to={latestHref}
        className="inline-flex shrink-0 items-center justify-center rounded-xl bg-warning px-3 py-1.5 text-xs font-medium text-warning-foreground hover:bg-warning/90"
      >
        View v{pkg.latestVersion}
      </Link>
    </div>
