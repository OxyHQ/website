      ...
      {isOpen && (
        <div>
          {feature.values.map((val, i) => (
            <div key={i} className="grid grid-cols-2 items-start border-border border-b py-2.5 text-sm text-muted-foreground">
              <span className="text-muted-foreground">{planNames[i]}</span>
              {renderValue(val)}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="pt-12 lg:hidden">
      <div className="flex flex-col items-center text-center">
        ...
        <div className="rounded-xl bg-surface p-0.5 mt-5">
      ...
      <div className="mt-10 flex flex-col gap-y-10">
        {featureCategories.map((category) => (
          <div key={category.name}>
            <h3 className="border-border border-b py-4 text-lg text-foreground">{category.name}</h3>
