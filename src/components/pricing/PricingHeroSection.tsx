  ...
  return (
    <div className="container">
      <div className="flex flex-col items-center pt-[116px]">
        ...
        <p className="mt-4 max-w-md text-balance text-center text-foreground text-xl">
      ...
      <div className="relative grid grid-cols-12 gap-x-6 mt-10 lg:mt-20">
        ...
        <div className="pricing-cards-grid-line bg-[linear-gradient(to_left,_transparent_0%,_var(--color-input)_6.52%,_var(--color-input)_93.22%,_transparent_100%)] absolute -top-3 left-0 hidden h-px w-full -translate-y-1/2 lg:block" />
        <div className="pricing-cards-grid-line bg-[linear-gradient(to_left,_transparent_0%,_var(--color-input)_6.52%,_var(--color-input)_93.22%,_transparent_100%)] absolute -bottom-3 left-0 hidden h-px w-full translate-y-1/2 [animation-delay:450ms]! lg:block" />
        ...
        <div className="relative isolate col-span-12 grid grid-cols-1 gap-6 lg:grid-cols-4 xl:col-span-10 xl:col-start-2">
          ...
          {[1, 2, 3, 4].map((col, idx) => (
            <div
              key={`vline-${col}`}
              className={`pricing-cards-grid-line-vertical bg-[linear-gradient(to_bottom,_transparent_0%,_var(--color-input)_10.87%,_var(--color-input)_89.55%,_transparent_100%)] absolute top-[-50px] -left-3 col-start-${col} hidden h-[calc(100%+100px)] w-px -translate-x-1/2 ${idx > 0 ? `[animation-delay:${idx * 150}ms]!` : ''} lg:block`}
            />
          ))}
          {/* Right edge line */}
          <div className="pricing-cards-grid-line-vertical bg-[linear-gradient(to_bottom,_transparent_0%,_var(--color-input)_10.87%,_var(--color-input)_89.55%,_transparent_100%)] absolute top-[-50px] -right-3 col-start-5 hidden h-[calc(100%+100px)] w-px translate-x-1/2 [animation-delay:600ms]! lg:block" />
          ...
          {[
            ...
            <div key={`corner-${i}`} className={`absolute ${pos} hidden lg:block`}>
              <div className="relative h-[7px] w-[7px] fade-in animate-in fill-mode-both [animation-delay:1000ms] [animation-duration:1000ms]">
                <div className="absolute top-[3px] left-0 h-px w-full rounded-full bg-muted" />
                <div className="absolute top-0 left-[3px] h-full w-px rounded-full bg-muted" />
              </div>
            </div>
          ))}
          ...
          {pricingPlans.map((plan) => (
            <div
              ...
              className={`flex flex-col justify-between rounded-3xl border border-solid px-[23px] pt-[21px] pb-[23px] ${
                plan.highlighted
                  ? 'border-primary/50 shadow-[0px_0px_0px_4px_rgba(196,_110,_222,_0.10),_0px_1px_2px_-1px_rgba(28,_40,_64,_0.10),_0px_2px_4px_0px_rgba(28,_40,_64,_0.06)]'
                  : 'border-border shadow-[0px_1px_2px_-1px_rgba(28,_40,_64,_0.08),_0px_2px_4px_0px_rgba(28,_40,_64,_0.04)]'
              }`}
            >
              <div className="flex flex-col">
                <header className="text-foreground text-xl">{plan.name}</header>
                <div className="mt-4 lg:mt-8">
                  <div className="flex items-start gap-2">
                    ...
                    {plan.price && plan.price.annual < plan.price.monthly && isAnnual && (
                      <div className="rounded-lg border border-primary/20 bg-primary/10 px-[7px] py-[3px] text-center text-primary text-xs mt-[6px]">
                        Save 20%
                      </div>
                    )}
                  </div>
              ...
              <div className="col-[2/-2]">
                <div className="grid max-w-7xl grid-cols-6 gap-x-8 gap-y-4.5 max-xl:grid-cols-4 max-xl:gap-y-5 max-lg:gap-x-5 max-lg:gap-y-3 max-md:grid-cols-3 max-md:gap-x-4 max-md:gap-y-2 max-md:[&>*:nth-child(1n+10)]:hidden">
                  {['Railway', 'Public', 'Wordsmith', 'Plain', 'Passionfroot'].map(name => (
                    <span key={name} className="size-full h-12 object-contain max-xl:h-10 text-sm font-medium text-muted-foreground flex items-center justify-center">{name}</span>
                  ))}
                </div>
              </div>
            </div>
