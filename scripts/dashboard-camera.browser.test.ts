...
try {
  ...
  await page.addInitScript(() => { window.__cameraFrames = []; });
  ...
  await page.waitForFunction(() => window.__cameraFrames.length > 20, { timeout: 60_000 });
  await page.evaluate(() => {
    window.setCameraEvents([{ service: 'mention', region: 'us-east-1', sourceRegion: 'edge-mad', sourceCoordinates: [150, -35], requests: 100, activityType: 'api', scope: 'external', direction: 'inbound', emittedAt: new Date().toISOString() }]);
  });
  ...
  const during = await page.evaluate(() => window.__cameraFrames.length);
  await page.waitForTimeout(200);
  assert.equal(await page.evaluate(() => window.__cameraFrames.length), during, 'automatic camera yields to drag');
  await page.mouse.up();
  const released = await page.evaluate(() => window.__cameraFrames.length);
  await page.waitForTimeout(1_800);
  assert.equal(await page.evaluate(() => window.__cameraFrames.length), released, 'manual view remains still for two seconds');
  await page.waitForFunction((count) => window.__cameraFrames.length > count, released);
